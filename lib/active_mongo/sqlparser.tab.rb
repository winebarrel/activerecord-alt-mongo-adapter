#
# DO NOT MODIFY!!!!
# This file is automatically generated by racc 1.4.5
# from racc grammer file "sqlparser.y".
#

require 'racc/parser'



require 'strscan'

module ActiveMongo


class SQLParser < Racc::Parser

module_eval <<'..end sqlparser.y modeval..idbd9d61f773', 'sqlparser.y', 205

KEYWORDS = %w(
  AND
  AS
  ASC
  BY
  COUNT
  DELETE
  DESC
  EXISTS
  FROM
  IN
  INSERT
  INTO
  LIMIT
  MOD
  NOT
  OFFSET
  ORDER
  REGEXP
  SELECT
  SET
  UPDATE
  VALUES
  WHERE
)

KEYWORD_REGEXP = Regexp.compile("(?:#{KEYWORDS.join '|'})\\b", Regexp::IGNORECASE)

def initialize(obj)
  src = obj.is_a?(IO) ? obj.read : obj.to_s
  @ss = StringScanner.new(src)
end

def scan
  piece = nil

  until @ss.eos?
    if (tok = @ss.scan /\s+/)
      # nothing to do
    elsif (tok = @ss.scan /(?:<>|!=|>=|<=|>|<|=)/)
      yield tok, tok
    elsif (tok = @ss.scan KEYWORD_REGEXP)
      yield tok.upcase.to_sym, tok
    elsif (tok = @ss.scan /NULL\b/i)
      yield :NULL, nil
    elsif (tok = @ss.scan /'(?:[^']|'')*'/) #'
      yield :STRING, tok.slice(1...-1).gsub(/''/, "'")
    elsif (tok = @ss.scan /-?(?:0|[1-9]\d*)(?:\.\d+)/)
      yield :NUMBER, tok.to_f
    elsif (tok = @ss.scan /-?(?:0|[1-9]\d*)/)
      yield :NUMBER, tok.to_i
    elsif (tok = @ss.scan /[,\(\)\*]/)
      yield tok, tok
    elsif (tok = @ss.scan /(?:[a-z_]\w+\.|[a-z]\.)*ID\b/i)
      yield :ID, tok
    elsif (tok = @ss.scan /(?:[a-z_]\w+\.|[a-z]\.)*(?:[a-z_]\w+|[a-z])/i)
      yield :IDENTIFIER, tok
    else
      raise Racc::ParseError, ('parse error on value "%s"' % @ss.rest.inspect)
    end
  end

  yield false, '$'
end
private :scan

def parse
  yyparse self, :scan
end

..end sqlparser.y modeval..idbd9d61f773

##### racc 1.4.5 generates ###

racc_reduce_table = [
 0, 0, :racc_error,
 1, 42, :_reduce_none,
 1, 42, :_reduce_none,
 1, 42, :_reduce_none,
 1, 42, :_reduce_none,
 10, 43, :_reduce_5,
 8, 44, :_reduce_6,
 8, 44, :_reduce_7,
 4, 55, :_reduce_8,
 6, 55, :_reduce_9,
 1, 56, :_reduce_none,
 1, 56, :_reduce_none,
 1, 50, :_reduce_12,
 1, 50, :_reduce_none,
 0, 51, :_reduce_14,
 2, 51, :_reduce_15,
 2, 51, :_reduce_16,
 1, 57, :_reduce_none,
 3, 57, :_reduce_18,
 3, 59, :_reduce_19,
 5, 59, :_reduce_20,
 1, 58, :_reduce_21,
 3, 58, :_reduce_22,
 1, 61, :_reduce_none,
 3, 61, :_reduce_24,
 3, 62, :_reduce_25,
 4, 62, :_reduce_26,
 5, 62, :_reduce_27,
 1, 62, :_reduce_none,
 6, 64, :_reduce_29,
 0, 52, :_reduce_30,
 4, 52, :_reduce_31,
 0, 65, :_reduce_32,
 1, 65, :_reduce_none,
 0, 53, :_reduce_34,
 2, 53, :_reduce_35,
 0, 54, :_reduce_36,
 2, 54, :_reduce_37,
 5, 45, :_reduce_38,
 1, 67, :_reduce_none,
 3, 67, :_reduce_40,
 3, 68, :_reduce_41,
 4, 46, :_reduce_42,
 1, 47, :_reduce_none,
 1, 48, :_reduce_44,
 3, 48, :_reduce_45,
 1, 60, :_reduce_none,
 1, 60, :_reduce_none,
 1, 60, :_reduce_none,
 1, 49, :_reduce_49,
 3, 49, :_reduce_50,
 1, 63, :_reduce_51,
 1, 63, :_reduce_52,
 1, 63, :_reduce_53,
 1, 63, :_reduce_54,
 1, 63, :_reduce_55,
 1, 63, :_reduce_56,
 1, 63, :_reduce_57,
 1, 63, :_reduce_58,
 1, 63, :_reduce_59,
 1, 63, :_reduce_60,
 1, 63, :_reduce_61,
 1, 66, :_reduce_62,
 1, 66, :_reduce_63 ]

racc_reduce_n = 64

racc_shift_n = 127

racc_action_table = [
    92,    19,     4,    14,    95,    33,    38,     7,    64,    65,
    91,    69,    72,    11,    74,    79,    33,    62,    41,    11,
    69,    72,    11,     2,    60,    61,     6,    75,    77,    78,
    66,    67,    68,    70,    71,    73,    75,    77,    78,    66,
    67,    68,    70,    71,    73,    49,    81,    81,    81,   120,
   121,    26,   116,   118,    45,    45,   124,    63,    51,    51,
    51,    51,   126,    33,    25,    79,    24,    62,    11,    11,
    11,    11,   115,   115,    60,    61,   115,    25,    62,    11,
    62,    84,   115,    62,    84,    60,    61,    60,    61,    62,
    60,    61,    62,    87,    62,    88,    60,    61,    62,    60,
    61,    60,    61,    27,    90,    60,    61,    57,    11,    11,
    11,    21,    96,    20,    98,    99,    99,    43,   103,    28,
   106,    11,    11,    11,   110,   111,   111,    11,    13,    33,
    12,    32,   123,    11,    11,    11 ]

racc_action_check = [
    76,     7,     0,     7,    80,    29,    27,     0,    45,    45,
    74,    47,    47,    41,    47,    80,    37,    76,    29,     7,
    83,    83,    27,     0,    76,    76,     0,    47,    47,    47,
    47,    47,    47,    47,    47,    47,    83,    83,    83,    83,
    83,    83,    83,    83,    83,    33,    49,    79,    81,   109,
   109,    18,   105,   107,    33,    49,   114,    44,    33,    49,
    79,    81,   117,    35,    17,    48,    15,    90,    33,    49,
    79,    81,   105,   107,    90,    90,   114,    44,    97,    51,
    92,    55,   117,   103,    56,    97,    97,    92,    92,   106,
   103,   103,    64,    57,   115,    63,   106,   106,    43,    64,
    64,   115,   115,    19,    65,    43,    43,    39,    13,    12,
    32,    10,    82,     8,    84,    85,    86,    31,    88,    20,
    91,    26,    25,    98,    99,   100,   101,    24,     6,    23,
     4,    22,   111,     2,    21,    87 ]

racc_action_pointer = [
     0,   nil,   106,   nil,   127,   nil,   120,    -8,   113,   nil,
    87,   nil,    82,    81,   nil,    58,   nil,    39,    43,    99,
   119,   107,   127,   117,   100,    95,    94,    -5,   nil,    -7,
   nil,   103,    83,    41,   nil,    51,   nil,     4,   nil,   102,
   nil,   -14,   nil,    77,    52,    -6,   nil,    -3,    49,    42,
   nil,    52,   nil,   nil,   nil,    63,    66,    83,   nil,   nil,
   nil,   nil,   nil,    89,    71,   100,   nil,   nil,   nil,   nil,
   nil,   nil,   nil,   nil,    -5,   nil,    -4,   nil,   nil,    43,
    -1,    44,   107,     6,    95,    95,    96,   108,   114,   nil,
    46,   116,    59,   nil,   nil,   nil,   nil,    57,    96,   103,
   103,   104,   nil,    62,   nil,    47,    68,    48,   nil,    10,
   nil,   111,   nil,   nil,    51,    73,   nil,    57,   nil,   nil,
   nil,   nil,   nil,   nil,   nil,   nil,   nil ]

racc_action_default = [
   -64,    -2,   -64,    -3,   -64,    -4,   -64,   -64,   -64,    -1,
   -64,   -43,   -64,   -64,   -12,   -64,   -44,   -13,   -64,   -64,
   -64,   -64,   -64,   -14,   -64,   -64,   -64,   -64,   127,   -14,
   -39,   -64,   -64,   -64,   -42,   -14,   -45,   -14,   -10,   -64,
   -11,   -64,   -38,   -64,   -64,   -64,   -15,   -64,   -16,   -64,
   -17,   -64,   -21,   -23,   -28,   -30,   -30,    -8,   -40,   -41,
   -46,   -48,   -47,   -64,   -64,   -64,   -55,   -56,   -57,   -61,
   -58,   -59,   -51,   -60,   -64,   -52,   -64,   -53,   -54,   -64,
   -64,   -64,   -64,   -64,   -64,   -34,   -34,   -64,   -64,   -19,
   -64,   -64,   -64,   -25,   -22,   -24,   -18,   -64,   -64,   -64,
   -36,   -36,    -9,   -64,   -49,   -64,   -64,   -64,   -26,   -32,
   -35,   -64,    -7,    -6,   -64,   -64,   -20,   -64,   -27,   -33,
   -62,   -63,   -31,   -37,    -5,   -50,   -29 ]

racc_goto_table = [
    10,    76,    17,    34,    50,    16,    30,    59,    18,    42,
    22,    23,   100,   101,     5,    55,   105,    56,   107,    31,
    82,    15,    35,    36,    37,    40,    58,    44,    89,   114,
    16,    39,   117,    85,    86,   112,   113,    97,    46,    31,
    93,    48,     3,     1,    94,     9,   122,   119,    29,    83,
     8,   nil,   nil,   nil,   nil,   nil,   nil,   nil,   nil,   nil,
   nil,   108,   nil,   nil,   nil,   nil,   nil,   nil,   nil,   nil,
   nil,   nil,   nil,   nil,   nil,   nil,   nil,   nil,   nil,   125,
   nil,   nil,   nil,   nil,   nil,   102,   nil,   nil,   nil,   nil,
   nil,   nil,   nil,   nil,   nil,   nil,   109 ]

racc_goto_check = [
     6,    22,     7,    10,    18,     6,    27,    19,     9,    10,
     6,     6,    12,    12,     5,    10,     8,    10,     8,     6,
    18,    14,     6,     6,     6,     6,    27,     7,    19,     8,
     6,    15,     8,    11,    11,    13,    13,    22,    16,     6,
    19,    17,     4,     3,    20,     2,    24,    25,    26,     6,
     1,   nil,   nil,   nil,   nil,   nil,   nil,   nil,   nil,   nil,
   nil,    19,   nil,   nil,   nil,   nil,   nil,   nil,   nil,   nil,
   nil,   nil,   nil,   nil,   nil,   nil,   nil,   nil,   nil,    19,
   nil,   nil,   nil,   nil,   nil,     6,   nil,   nil,   nil,   nil,
   nil,   nil,   nil,   nil,   nil,   nil,     6 ]

racc_goto_pointer = [
   nil,    50,    45,    43,    42,    14,    -2,    -5,   -74,     1,
   -20,   -22,   -73,   -65,    14,     4,     5,     8,   -29,   -36,
   -35,   nil,   -46,   nil,   -63,   -62,    27,   -15 ]

racc_goto_default = [
   nil,   nil,   nil,   nil,   nil,   nil,    47,   nil,   nil,   nil,
   nil,   nil,   nil,   nil,   nil,   nil,   nil,    80,   nil,   104,
    52,    53,   nil,    54,   nil,   nil,   nil,   nil ]

racc_token_table = {
 false => 0,
 Object.new => 1,
 :INSERT => 2,
 :INTO => 3,
 "(" => 4,
 ")" => 5,
 :VALUES => 6,
 :SELECT => 7,
 :FROM => 8,
 :COUNT => 9,
 :AS => 10,
 "*" => 11,
 :WHERE => 12,
 :ID => 13,
 "=" => 14,
 :IN => 15,
 :AND => 16,
 :NOT => 17,
 :ORDER => 18,
 :BY => 19,
 :LIMIT => 20,
 :NUMBER => 21,
 :OFFSET => 22,
 :UPDATE => 23,
 :SET => 24,
 "," => 25,
 :DELETE => 26,
 :IDENTIFIER => 27,
 :STRING => 28,
 :NULL => 29,
 :REGEXP => 30,
 :MOD => 31,
 :EXISTS => 32,
 "<>" => 33,
 "!=" => 34,
 ">=" => 35,
 "<=" => 36,
 ">" => 37,
 "<" => 38,
 :ASC => 39,
 :DESC => 40 }

racc_use_result_var = false

racc_nt_base = 41

Racc_arg = [
 racc_action_table,
 racc_action_check,
 racc_action_default,
 racc_action_pointer,
 racc_goto_table,
 racc_goto_check,
 racc_goto_default,
 racc_goto_pointer,
 racc_nt_base,
 racc_reduce_table,
 racc_token_table,
 racc_shift_n,
 racc_reduce_n,
 racc_use_result_var ]

Racc_token_to_s_table = [
'$end',
'error',
'INSERT',
'INTO',
'"("',
'")"',
'VALUES',
'SELECT',
'FROM',
'COUNT',
'AS',
'"*"',
'WHERE',
'ID',
'"="',
'IN',
'AND',
'NOT',
'ORDER',
'BY',
'LIMIT',
'NUMBER',
'OFFSET',
'UPDATE',
'SET',
'","',
'DELETE',
'IDENTIFIER',
'STRING',
'NULL',
'REGEXP',
'MOD',
'EXISTS',
'"<>"',
'"!="',
'">="',
'"<="',
'">"',
'"<"',
'ASC',
'DESC',
'$start',
'sql',
'create_statement',
'read_statemant',
'update_statemant',
'delete_statemant',
'id',
'id_list',
'value_list',
'select_list',
'where_clause',
'order_by_clause',
'limit_clause',
'offset_clause',
'count_clause',
'count_arg',
'id_search_condition',
'search_condition',
'id_predicate',
'value',
'boolean_primary',
'predicate',
'op',
'not_in_predicate',
'ordering_spec',
'order_spec',
'set_clause_list',
'set_clause']

Racc_debug_parser = false

##### racc system variables end #####

 # reduce 0 omitted

 # reduce 1 omitted

 # reduce 2 omitted

 # reduce 3 omitted

 # reduce 4 omitted

module_eval <<'.,.,', 'sqlparser.y', 12
  def _reduce_5( val, _values)
                            {:command => :insert, :table => val[2], :column_list => val[4], :value_list => val[8]}
  end
.,.,

module_eval <<'.,.,', 'sqlparser.y', 17
  def _reduce_6( val, _values)
                            {:command => :select, :table => val[3], :select_list => val[1], :condition => val[4], :order => val[5], :limit => val[6], :offset => val[7]}
  end
.,.,

module_eval <<'.,.,', 'sqlparser.y', 21
  def _reduce_7( val, _values)
                            {:command => :select, :table => val[3], :count => val[1], :condition => val[4], :order => val[5], :limit => val[6], :offset => val[7]}
  end
.,.,

module_eval <<'.,.,', 'sqlparser.y', 26
  def _reduce_8( val, _values)
                            "count_all"
  end
.,.,

module_eval <<'.,.,', 'sqlparser.y', 30
  def _reduce_9( val, _values)
                            val[5]
  end
.,.,

 # reduce 10 omitted

 # reduce 11 omitted

module_eval <<'.,.,', 'sqlparser.y', 38
  def _reduce_12( val, _values)
                            []
  end
.,.,

 # reduce 13 omitted

module_eval <<'.,.,', 'sqlparser.y', 44
  def _reduce_14( val, _values)
                            []
  end
.,.,

module_eval <<'.,.,', 'sqlparser.y', 48
  def _reduce_15( val, _values)
                            val[1]
  end
.,.,

module_eval <<'.,.,', 'sqlparser.y', 52
  def _reduce_16( val, _values)
                            val[1]
  end
.,.,

 # reduce 17 omitted

module_eval <<'.,.,', 'sqlparser.y', 58
  def _reduce_18( val, _values)
                            val[1]
  end
.,.,

module_eval <<'.,.,', 'sqlparser.y', 63
  def _reduce_19( val, _values)
                            val[2]
  end
.,.,

module_eval <<'.,.,', 'sqlparser.y', 67
  def _reduce_20( val, _values)
                            val[3]
  end
.,.,

module_eval <<'.,.,', 'sqlparser.y', 72
  def _reduce_21( val, _values)
                            [val[0]].flatten
  end
.,.,

module_eval <<'.,.,', 'sqlparser.y', 76
  def _reduce_22( val, _values)
                            (val[0] << val[2]).flatten
  end
.,.,

 # reduce 23 omitted

module_eval <<'.,.,', 'sqlparser.y', 82
  def _reduce_24( val, _values)
                            val[1]
  end
.,.,

module_eval <<'.,.,', 'sqlparser.y', 87
  def _reduce_25( val, _values)
                            {:name => val[0], :op => val[1], :expr => val[2]}
  end
.,.,

module_eval <<'.,.,', 'sqlparser.y', 91
  def _reduce_26( val, _values)
                            {:name => val[1], :op => val[2], :expr => val[3], :not => true}
  end
.,.,

module_eval <<'.,.,', 'sqlparser.y', 95
  def _reduce_27( val, _values)
                            {:name => val[0], :op => val[1], :expr => val[3]}
  end
.,.,

 # reduce 28 omitted

module_eval <<'.,.,', 'sqlparser.y', 101
  def _reduce_29( val, _values)
                            {:name => val[0], :op => '$nin', :expr => val[4]}
  end
.,.,

module_eval <<'.,.,', 'sqlparser.y', 106
  def _reduce_30( val, _values)
                            nil
  end
.,.,

module_eval <<'.,.,', 'sqlparser.y', 110
  def _reduce_31( val, _values)
                            {:name => val[2], :type => val[3]}
  end
.,.,

module_eval <<'.,.,', 'sqlparser.y', 115
  def _reduce_32( val, _values)
                            :asc
  end
.,.,

 # reduce 33 omitted

module_eval <<'.,.,', 'sqlparser.y', 121
  def _reduce_34( val, _values)
                            nil
  end
.,.,

module_eval <<'.,.,', 'sqlparser.y', 125
  def _reduce_35( val, _values)
                            val[1]
  end
.,.,

module_eval <<'.,.,', 'sqlparser.y', 130
  def _reduce_36( val, _values)
                            nil
  end
.,.,

module_eval <<'.,.,', 'sqlparser.y', 134
  def _reduce_37( val, _values)
                            val[1]
  end
.,.,

module_eval <<'.,.,', 'sqlparser.y', 139
  def _reduce_38( val, _values)
                            {:command => :update, :table => val[1], :set_clause_list => val[3], :condition => val[4]}
  end
.,.,

 # reduce 39 omitted

module_eval <<'.,.,', 'sqlparser.y', 145
  def _reduce_40( val, _values)
                            val[0].merge val[2]
  end
.,.,

module_eval <<'.,.,', 'sqlparser.y', 150
  def _reduce_41( val, _values)
                          {val[0] => val[2]}
  end
.,.,

module_eval <<'.,.,', 'sqlparser.y', 155
  def _reduce_42( val, _values)
                            {:command => :delete, :table => val[2], :condition => val[3]}
  end
.,.,

 # reduce 43 omitted

module_eval <<'.,.,', 'sqlparser.y', 162
  def _reduce_44( val, _values)
                            [val[0]]
  end
.,.,

module_eval <<'.,.,', 'sqlparser.y', 166
  def _reduce_45( val, _values)
                            val[0] << val[2]
  end
.,.,

 # reduce 46 omitted

 # reduce 47 omitted

 # reduce 48 omitted

module_eval <<'.,.,', 'sqlparser.y', 175
  def _reduce_49( val, _values)
                            [val[0]]
  end
.,.,

module_eval <<'.,.,', 'sqlparser.y', 179
  def _reduce_50( val, _values)
                            val[0] << val[2]
  end
.,.,

module_eval <<'.,.,', 'sqlparser.y', 180
  def _reduce_51( val, _values)
 '$in'
  end
.,.,

module_eval <<'.,.,', 'sqlparser.y', 181
  def _reduce_52( val, _values)
 '$regexp'
  end
.,.,

module_eval <<'.,.,', 'sqlparser.y', 182
  def _reduce_53( val, _values)
 '$mod'
  end
.,.,

module_eval <<'.,.,', 'sqlparser.y', 183
  def _reduce_54( val, _values)
 '$exists'
  end
.,.,

module_eval <<'.,.,', 'sqlparser.y', 184
  def _reduce_55( val, _values)
 '$ne'
  end
.,.,

module_eval <<'.,.,', 'sqlparser.y', 185
  def _reduce_56( val, _values)
 '$ne'
  end
.,.,

module_eval <<'.,.,', 'sqlparser.y', 186
  def _reduce_57( val, _values)
 '$gte'
  end
.,.,

module_eval <<'.,.,', 'sqlparser.y', 187
  def _reduce_58( val, _values)
 '$lte'
  end
.,.,

module_eval <<'.,.,', 'sqlparser.y', 188
  def _reduce_59( val, _values)
 '$gt'
  end
.,.,

module_eval <<'.,.,', 'sqlparser.y', 189
  def _reduce_60( val, _values)
 '$lt'
  end
.,.,

module_eval <<'.,.,', 'sqlparser.y', 190
  def _reduce_61( val, _values)
 '$eq'
  end
.,.,

module_eval <<'.,.,', 'sqlparser.y', 192
  def _reduce_62( val, _values)
 :asc
  end
.,.,

module_eval <<'.,.,', 'sqlparser.y', 193
  def _reduce_63( val, _values)
 :desc
  end
.,.,

 def _reduce_none( val, _values)
  val[0]
 end

end   # class SQLParser


end # module ActiveMongo
